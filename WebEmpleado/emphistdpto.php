...
<BODY>
<h1>HISTORIAL EMPLEADOS DEPARTAMENTOS</h1>
...
function cargar_empleados($conn, $departamento){
	$array = array();
	$stmt = $conn->prepare("SELECT dni FROM emple_depart WHERE cod_dpto = :departamento AND fecha_fin IS NOT NULL");
...
function mostrar_tabla($array){
	if(!empty($array)){
		...
		$resultado = "No hay empleados dados de baja en ese departamento";
		return $resultado;
	}
